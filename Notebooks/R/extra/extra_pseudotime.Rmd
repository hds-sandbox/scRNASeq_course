...
sample.combined.sct <- LoadH5Seurat("/work/Intro_to_scRNAseq_R/Results/sample_23.filt.norm.red.clust.h5Seurat")
```
...
sde <- slingshot(sample.combined.SCE, reducedDim = 'UMAP')
```
...
plot(reducedDim(sample.combined.SCE, "UMAP"), pch=16, col = cols) # sample.combined.SCE contains, PC1 and PC2, and colored by pseudotime
...
FeaturePlot(sample.combined.sct, features = "pseudotime", reduction = "umap") + scale_color_viridis_c()# + geom_path(data = trajectory, aes(x = PC_1, y = PC_2))
```
