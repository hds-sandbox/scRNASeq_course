...
![](./img/Part04_clustering/spermatogenesis.png)
...
**Learning objectives:**
- Perform unsupervised clustering on the dataset
...
sample_2 <- FindClusters(sample_2, resolution = 0.5)
```

The clusters can be found using the `Idents()` function or in the metadata of the assay. Specifically, each time that you run `FindClusters()` with different resolutions, a new cluster results column specifying the resolution used will appear in the `metadata` object. The metadata column `seurat_clusters` will have the latest calculated clusters, and the `Ident` of the seurat object will be updated.

```{r}
head(Idents(sample_2), 5)
```
...
```{r}
head(sample_2$seurat_clusters, 5)
```
...
**Note**: the function `FindAllMarkers()` can take several minutes with default options. Depending on the size of the dataset, we recommend some of the options below.

```{r}
# Find all markers
all.markers <- FindAllMarkers(sample_2)
...
pos.markers <- FindAllMarkers(sample_2, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
pos.markers %>% group_by(cluster) %>% slice_max(n = 2, order_by = avg_log2FC) # Show top 2 markers for each cluster
```

You can easily save your marker dataframes using `write.table()`

```{r}
write.table(x = all.markers, file = "/work/Intro_scRNAseq_R/Results/sample_2_all_markers.tsv", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
```
...
```{r}
cluster0.markers <- FindMarkers(sample_2, ident.1 = 0, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
```
...
DoHeatmap(sample_2, features = top10$gene) + NoLegend()
```
...
Now that we know what kind of cell types our clusters are, we can name them properly. Write the names in the list `new_names`. You should be able to **give a name for each cell type**. Below is an example, so you will need to match it to your cluster results. If there is more than one cluster with same cell type, just write the name followed by a dot `.` and a number. For example, by writing for example `.1` and `.2` at the end of the names. We will remove the numbers afterwords.
...
```{r}
av.exp <- AverageExpression(sample_2, assay = "SCT", slot = "data")
av.exp <- av.exp$SCT
...
```{r}
SaveH5Seurat(sample_2, filename = "/work/Intro_scRNAseq_R/Results/sample_2.filt.norm.red.clust.h5Seurat")
```
